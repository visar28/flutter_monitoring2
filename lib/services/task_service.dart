...
class TaskService {
  ...
  Future<void> generateDummyData() async {
    try {
      ...
      for (DateTime date = twoMonthsAgo; date.isBefore(now); date = date.add(Duration(days: 1))) {
        ...
        // Generate 5-15 tasks per day
        final tasksPerDay = 5 + random.nextInt(11);
        
        Map<String, dynamic> dailyTasks = {};
        
        for (int i = 1; i <= tasksPerDay; i++) {
          ...
          final finalStatus = random.nextDouble() < 0.6 ? 'Close' : status;
          
          dailyTasks['task_$i'] = {
            'no': i,
            'wo': 'WO-${dateStr.replaceAll('-', '')}-${i.toString().padLeft(3, '0')}',
            'desc': 'Task $typeWO untuk $category - ${_generateTaskDescription()}',
            'typeWO': typeWO,
            'pic': pic,
            'status': finalStatus,
            'category': category,
        ...
        print('Generated ${tasksPerDay} tasks for ${dateStr}');
      }
  ...
  String _generateTaskDescription() {
    final descriptions = [
  ...
  Future<Map<String, dynamic>> getTasksForDate(String date) async {
    try {
      ...
      print('Error getting tasks for date $date: $e');
      return {};
    }
  }
  
  // Get tasks for date range
  Future<Map<String, Map<String, dynamic>>> getTasksForDateRange(
    DateTime startDate, 
    DateTime endDate
  ) async {
    try {
      Map<String, Map<String, dynamic>> allTasks = {};
  ...
  // Calculate PIC performance for date range
  Future<Map<String, Map<String, dynamic>>> calculatePICPerformance(
    ...
    try {
      ...
      // Process all tasks
      allTasks.forEach((date, dailyTasks) {
        dailyTasks.forEach((taskKey, taskData) {
          if (taskData is Map<String, dynamic>) {
            final pic = taskData['pic']?.toString() ?? '';
            final status = taskData['status']?.toString() ?? '';
            
            if (pic.isNotEmpty && pic != 'admin') {
  ...
  Future<void> addTask(String date, Map<String, dynamic> taskData) async {
    try {
      final docRef = _firestore.collection('tactical_work_orders').doc(date);
      ...
      print('✅ Task added successfully for date: $date');
  ...
  // Update task status (only allowed on task date)
  ...
  // Process Excel data with PIC filtering and performance calculation
  Future<void> processExcelData(
    List<List<dynamic>> excelRows, 
    String targetDate
  ) async {
    try {
      print('🔄 Processing Excel data for date: $targetDate');
      ...
      if (processedTasks.isNotEmpty) {
        ...
        print('✅ Excel data processed and saved: ${processedTasks.length} tasks');
        
        // Log PIC performance summary
        print('📊 PIC Performance Summary:');
  ...
  bool canEditTasksForDate(String date, bool isAdmin) {
    if (isAdmin) {
      // Admin can only edit today's tasks
      final today = DateTime.now();
      final taskDate = DateTime.parse(date);
      ...
      // Members can only edit today's tasks
      final today = DateTime.now();
      final taskDate = DateTime.parse(date);
      return taskDate.year == today.year && 
             taskDate.month == today.month && 
  ...
  // Get available dates for task viewing (last 7 days for members, all for admin)
